...
Defines the Wayland parity-testing surface: the test-only `nowayprompt-wayland-test` binary for driving the Wayland frontend directly, and the deterministic automated compositor gate that compares the target against the pinned `pkgs.wayprompt` oracle on a layer-shell compositor.
